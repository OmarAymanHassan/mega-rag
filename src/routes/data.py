...
async def upload_data(request:Request,
    ...
    # Create asset model to store info about the uploaded file, name, size, and the project it belongs to
    ...
    return JSONResponse(
        content={
            ...
            "file_id": str(asset_record.id) # return the unique filename to be stored in db
    ...
    if process_request.file_id :
        ...
        if not asset_record:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={
                    "signal": ResponseSignal.ASSETS_FILE_ID_ERROR.value,
                }
            )         
        ...
        project_files = await asset_model.get_all_project_assets(asset_project_id=project.id, asset_type=AssetTypeEnums.FILE.value ) # Now i have all files of project_id 1 or 2 or 3

        logger.info(f"All assests for that project : {len(project_files)}")

        # extract assets_name for all files i have
        ...
        logger.info(f"Number of project_file_ids : {len(project_files_ids)}")
        logger.info(f"The project_id : {project.id}")

        
        # now for each file_id --> i do chunking

        #but we need to make sure that there is file_ids in first place

        if len(project_files_ids) ==0:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={
                    "signal": ResponseSignal.ASSETS_NOT_fOUND.value,
                }
            )
